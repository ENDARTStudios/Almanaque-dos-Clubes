...
Motivo: **merge #218** (`7411283`) → deploy Vercel **READY** (`l5iji0idn`, alias `almanaquedosclubes.com`/`www`). **Fetch cache-bypass:** HTTP **200**; SHA-256 `720695cce16c068b1982311cc8aff023a49a52280cc2ac94031fb69fbdd55805`. **Strings:** PR (`Campeonato Paranaense`, `Moacir Dalpiaz de Souza`, `pr2025.htm`, `proper acknowledgement … author`, `All rights reserved`, âncora `campeonato-paranaense-2025`); GO (`Guillermo Alexander Rivera`, `go2025.htm`, título 2023–2025); MG (`Claudio Freati`, `mg2025.htm`); “**Não é domínio público**” (negação) ×6; texto obsoleto “2025 não incluída” **removido**; **zero** segredo/config no HTML. **SQL (DB↔UI):** MG = Claudio Freati 2023–2025 (`uf` **null** — writer v1, observação) · GO = Guillermo Alexander Rivera 2023–2025 (`uf` GO) · PR = Moacir Dalpiaz de Souza 2025 (`uf` PR) → autores/anos **consistentes** com a página. Sem divergência factual. **T448b-2d (Estaduais Piloto) encerrado técnica e documentalmente.** Próxima prioridade: **T448b-2g** (backfill 9 clubes sem QID) ou **Discovery Nova UF**.

### [2026-09-25] Decisão: D-2026-09-25-t448b2d-metodologia-pr-atribuicao — Conformidade pública do piloto Paranaense 2025 restaurada

Motivo: a página `/metodologia` (web) documentava MG/GO/Inglaterra, mas **não** a atribuição RSSSF do **Paranaense 2025** (o crédito a Moacir Dalpiaz de Souza existia só no JSONB do banco). **Correção (texto/metodologia; sem CSS/lógica/rota):** nova seção `id="campeonato-paranaense-2025"` espelhando o padrão MG/GO — fonte `rsssfbrasil.com/tablesfq/pr2025.htm`; crédito **Moacir Dalpiaz de Souza**; licença verbatim (copyright + "proper acknowledgement is given to the author. All rights reserved."); nota **"(Não é domínio público.)"**; limitação (só elite masculina 2025; identidade por Wikidata QID `Q2580083`) e canal de correção. **Consistência DB↔UI (bônus):** a seção GO foi atualizada de **2023–2024** (texto dizia "2025 não incluída") para **2023–2025** (+ `go2025.htm`), pois o GO 2025 já está em produção. Datas → 25/09/2026. Build + render local confirmam as strings; lint/prettier 0.

### [2026-09-25] Decisão: D-2026-09-25-t448b2d-writers-gate-producao-concluido — Arestas WON GO 2025 e PR 2025 APLICADAS em produção

Motivo: **merge #216** (`e4ea209`) → deploy Railway **SUCCESS**. Artefatos no container (`go-2025/pr-2025-pilot-candidates.json`, `write-rsssf-won-edges.js`) com `clubQid` corretos (`Q1513287`/`Q2580083`). **DRY** `--pack=go-2025` e `--pack=pr-2025` → `created=1`, zeros, `errors=[]`. **APPLY `--allow-production`** (transação Serializable): `created=1` em cada; `hardDeletes=0`; `migrations=0`. **SQL:** total estadual RSSSF ativo **5 → 7**; `missing_provenance=0`; `go_2025_link_ok=1`; `pr_2025_link_ok=1`; baseline **MG=3 / GO 2023–24=2** intactos; `unexpected_soft_deleted=0`. Novas arestas: `Q931386|2025|Q1513287|WON` (writerVersion `t448b2d-writer-go-pr-v1`, uf GO, autor Rivera) e `Q920397|2025|Q2580083|WON` (uf PR, autor Moacir Dalpiaz de Souza). **Cache:** DEL cirúrgico `champions:all` (sem FLUSHALL/FLUSHDB). **Smoke:** `/clubs/<VN GO>/titles` e `/clubs/<OP PR>/titles` 200 com `go2025.htm`/`pr2025.htm`; `/champions` 200. Homônimos `Q10391045/Q10391046/Q671621` intocados. **Pendência (follow-up web):** `/metodologia` tem MG+GO, mas **ainda não** a seção de atribuição **PR** — abrir PR web dedicada (atribuição RSSSF ao autor Moacir Dalpiaz de Souza).
