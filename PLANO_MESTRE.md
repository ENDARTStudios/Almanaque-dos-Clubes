...
**Nota T448b-2b FASE 2 (09-25): [~] writer empacotado — BLOQUEADO por fix de proveniência.** Pack congelado em `src/lib/rsssf/data/mg-pilot-candidates.json` + `candidates-pack.ts` (Zod/fail-fast); writer lê o pack por padrão (`--from-fixtures` = dev); trava de produção só no `--apply`. **Gate de produção abortou no check 3.4** (writer v1 descartava `candidate.retrievedAt`); rollback lógico aplicado (3 soft-deleted) e fix em PR #198 (`retrievedAt` persistido + restore idempotente sobre soft-deleted + APPLY atômico). **NÃO marcar [x] até o gate de produção verde.** Expansão além de MG = packs novos/ingestão dinâmica (fora do round).
