...
**Nota T448b-2b FASE 2 (09-25): [x] técnica — writer empacotado p/ produção.** Pack congelado em `src/lib/rsssf/data/mg-pilot-candidates.json` + `candidates-pack.ts` (Zod/fail-fast); writer lê o pack por padrão (`--from-fixtures` = dev). Trava de produção só no `--apply`. Dry-run **dentro do container** provou o arquivo presente e `created=3`. **Gate de Produção condicionado ao merge #195+#196+#197 e revalidação dry-run em ambiente prod-like.** Expansão além de MG = packs novos/ingestão dinâmica (fora do round).
