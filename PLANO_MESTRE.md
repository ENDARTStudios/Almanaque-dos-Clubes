...
**Nota T448b-2f (09-25): [~] Identity Remediation (Opção A).** Removido `@@unique([name,country])` → índice não-único + validação contextual (`club-uniqueness.ts`); `clubsService.create` e `prisma/seed.ts` ajustados; homônimos nacionais coexistem (GO/RN), duplicata exata bloqueada. **Destrava GO 2025 / PR 2025.** Backfill dos **9 clubes sem QID = T448b-2g** (follow-up). **[x] após gate produção.**
