...
**Nota T448b-2d GO gate (09-25): apply BLOQUEADO por rate limit Vercel.** Writer GO + parser GO **mergeados** (#205/#206) e atribuição web **mergeada** (#207), mas o **deploy de produção Vercel do #207 não subiu** (`api-deployments-free-per-day`, ~24h) → `/metodologia` **sem** a seção GO → **apply GO abortado** (`blocked_reason=vercel_deploy_rate_limited_24h`). Estado congelado: GO=0, MG=3, total=3, homônimos intactos, Q1513287 ausente. Retomada após o deploy conter o #207. Sem contorno/bypass/divergência.
