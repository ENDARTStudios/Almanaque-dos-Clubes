...
**Nota T448b-2d seed identidades GO/PR 2025 (09-25): [x] CONCLUÍDO - identidades ativas em produção (adendum 35).** Micro-seed só-clubes por QID (`Q1513287` Vila Nova/GO, `Q2580083` Operário Ferroviário/PR) via Wikidata CC0: `lib/rsssf/seeds/club-seed.ts` + packs + script (DRY/`--apply`). Homônimos `Q10391045/Q10391046/Q671621` intocados. Testes unit 9 + integração 1. #214 (`4372cd8`) → apply prod `created=2`; clubes ativos 3878→3880; dup QID=0. **Writers GO/PR 2025 [x] CONCLUÍDOS — aplicados em produção (adendum 37).** `--pack=go-2025|pr-2025`; `created=1` cada; total estadual RSSSF **5 → 7**; baseline MG/GO intacto. **Conformidade pública [x]**: seção PR em `/metodologia` + GO 2023–2025; **validada ao vivo** (adendum 39). **T448b-2d ENCERRADO.**
...
**Nota T448b-2f (09-25): [x] Identity Remediation (Opção A) — APLICADO em produção.** Migration `t448b2f` aplicada no boot (deploy `85c0610`): `clubs_name_country_key` removido, `clubs_name_country_idx` não-único criado, `clubs_qid_key` intacto; homônimos=0; estadual RSSSF=5. **FASE 2 LIBERADA:** parsers GO 2025 (`Q1513287`) / PR 2025 (`Q2580083`). Follow-up **T448b-2g** (backfill 9 sem QID). (Histórico: [~] Identity Remediation (Opção A). Removido `@@unique([name,country])` → índice não-único + validação contextual (`club-uniqueness.ts`); `clubsService.create` e `prisma/seed.ts` ajustados; homônimos nacionais coexistem (GO/RN), duplicata exata bloqueada. **Destrava GO 2025 / PR 2025.** Backfill dos **9 clubes sem QID = T448b-2g** (follow-up). **[x] após gate produção.**

**Nota T448b-2d GO (09-25): [x] Piloto GO 2023–2024 ATIVO em produção.** Parser + writer + atribuição (#205/#206/#207) mergeados; rate limit Vercel **resetou** → deploy prod do web SUCCESS → `/metodologia` com atribuição GO → **apply GO: created=2** (2023/2024). SQL: GO=2, MG=3, **total estadual RSSSF=5**, `missing_provenance=0`, 2025=0, Q1513287=0, homônimos intactos. `/champions` determinístico (estadual=Atlético-MG 2025). **GO 2025 = gap** (`club_name_unique_conflict`). Próximo: **T448b-2f** (identidade/homônimos) → reavaliar GO 2025/PR/outras UFs.
