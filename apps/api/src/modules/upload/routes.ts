...
export const uploadRoutes: FastifyPluginAsync = async (app: FastifyInstance) => {
  app.post('/upload', { preHandler: [authenticate, requirePermission(PERMISSIONS.CLUBS_WRITE)] }, async (request, reply) => {
  ...
  app.post('/upload/csv', { preHandler: [authenticate, requirePermission(PERMISSIONS.CLUBS_MANAGE)] }, async (request, reply) => {
    const data = await request.file();
    if (!data) {
      return reply.status(400).send({ error: { code: 'NO_FILE', message: 'Nenhum arquivo CSV enviado' } });
    }
    if (data.mimetype !== 'text/csv' && !data.filename.endsWith('.csv')) {
      return reply.status(422).send({ error: { code: 'INVALID_FORMAT', message: 'Apenas arquivos CSV são aceitos' } });
    }
    const buffer = await data.toBuffer();
