...
export const graphRoutes: FastifyPluginAsync = async (app: FastifyInstance) => {
  app.get('/graph', async (_request, reply) => {
    try { return reply.send({ data: await graphService.getGraph() }); }
    catch (err) { return handleError(err, reply); }
  });

  app.get<{ Params: { entityType: string; entityId: string } }>('/graph/:entityType/:entityId', async (request, reply) => {
    try { return reply.send({ data: await graphService.getEdgesForEntity(request.params.entityId, request.params.entityType) }); }
    catch (err) { return handleError(err, reply); }
  });

  app.post('/graph', { preHandler: [authenticate, requirePermission(PERMISSIONS.CLUBS_MANAGE)] }, async (request, reply) => {
    try {
...
function handleError(err: unknown, reply: import('fastify').FastifyReply) {
  if (err instanceof DomainError) return reply.status(err.statusCode).send({ error: { code: err.code, message: err.message } });
  if (err instanceof ZodError) return reply.status(422).send({ error: { code: 'VALIDATION_ERROR', message: 'Payload inválido' } });
