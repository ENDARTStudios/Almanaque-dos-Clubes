  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <p className="text-sm text-foreground/50 mb-8">
        {t('legal.updatedLabel')}: 2026-09-19 (v1.2) · END ART Studios · CNPJ 45.370.930/0001-75
      </p>
      <p className="text-foreground/70 mb-8 leading-relaxed">{doc.intro}</p>

      <div className="space-y-8">
        {doc.sections.map((section) => (
          <section key={section.title} id={section.title.replace(/^\d+\.\s*/, '').toLowerCase().replace(/\s+/g, '-')}>
