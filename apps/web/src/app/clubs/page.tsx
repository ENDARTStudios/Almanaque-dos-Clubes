...
async function getClubs(searchParams: { [key: string]: string | undefined }) {
  ...
  params.set('limit', '50');
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api/v1'}/clubs?${params}`, { cache: 'no-store' });
...
export default async function ClubsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | undefined }> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      <h1 className="text-3xl sm:text-4xl font-heading font-bold text-foreground mb-2">Clubes</h1>
      <p className="text-foreground/60 mb-8">Explore {total} clubes de futebol cadastrados.</p>
      <div className="mb-8">
        <Suspense fallback={<div className="h-12 bg-gray-100 rounded-xl animate-pulse" />}>
          <SearchBar placeholder="Buscar clubes por nome, país ou cidade..." />
        </Suspense>
      </div>
